#!/usr/bin/env python3
"""
Dispatchwrapparr - Version 0.5.0 Beta: A wrapper for Dispatcharr that supports the following:
  ...
  - Teletext and Closed Caption support
Usage: dispatchwrapper.py -i <URL> -ua <User Agent String>
Optional: -proxy <proxy server> -proxybypass <proxy bypass list> -clearkeys <file/url> -loglevel <level> -subtitles -teletext -closed-captions
"""
...
@pluginargument(
    "use-teletext",
    action="store_true",
    help="Enable teletext support"
)
@pluginargument(
    "use-closed-captions",
    action="store_true",
    help="Enable closed captions support"
)
...
class FFMPEGMuxerDRM(FFMPEGMuxer):
    """
    Inherit and extend the FFMPEGMuxer class to pass decryption keys and subtitle options
    to ffmpeg
    """
    ...
    def __init__(self, session, *streams, **options):
        ...
        teletext = self.session.options.get("use-teletext")
        closed_captions = self.session.options.get("use-closed-captions")
        ...
        while len(old_cmd) > 0:
            ...
            elif teletext and cmd == "-c:a":
                _ = old_cmd.pop(0)
                self._cmd.extend([cmd, _])
                self._cmd.extend(["-scodec", "dvbsub"])  # Teletext codec
            elif closed_captions and cmd == "-c:a":
                _ = old_cmd.pop(0)
                self._cmd.extend([cmd, _])
                self._cmd.extend(["-c:s", "mov_text"])  # Closed captions codec
            else:
...
class DASHStreamDRM(DASHStream):
    ...
    def __init__(
        ...
        teletext_representations: List[Representation] | None = None,
        closed_captions_representations: List[Representation] | None = None,
        **kwargs,
    ):
        super().__init__(
            session,
            ...
            audio_representations[0] if audio_representations else None,
            **kwargs,
        )
        self.audio_representations = audio_representations
        self.subtitles_representations = subtitles_representations
        self.teletext_representations = teletext_representations
        self.closed_captions_representations = closed_captions_representations
    ...
    def parse_manifest(
        ...
        **kwargs,
        ...
        teletext: list[Representation | None] = [None] if with_audio_only else []
        closed_captions: list[Representation | None] = [None] if with_audio_only else []
        ...
        # Search for suitable video, audio, subtitles, teletext, and closed captions representations
        for aset in period_selection.adaptationSets:
            ...
            for rep in aset.representations:
                ...
                if rep.mimeType.startswith("video"):
                    ...
                    audio.append(rep)
                elif rep.mimeType.startswith("application") and rep.lang:
                    subtitles.append(rep)
                elif rep.mimeType.startswith("text") and "teletext" in rep.codecs.lower():
                    teletext.append(rep)
                elif rep.mimeType.startswith("text") and "cea-608" in rep.codecs.lower():
                    closed_captions.append(rep)
        ...
        if not teletext:
            teletext.append(None)
        if not closed_captions:
            closed_captions.append(None)
        ...
        for vid, aud in itertools.product(video, audio):
            if not vid and not aud:
                continue
            stream = DASHStreamDRM(session, mpd, vid, audio, subtitles, teletext, closed_captions, **kwargs)
            stream_name = []
            if vid:
    ...
    def open(self):
        ...
        rep_teletext = self.teletext_representations
        rep_closed_captions = self.closed_captions_representations
        timestamp = now()
        fds = []
        maps = ["0:v?", "0:a?"]
        metadata = {}
        ...
        if rep_teletext and rep_teletext[0] and rep_video:
            for _, rep_teletext_stream in enumerate(rep_teletext):
                teletext = DASHStreamReaderSUB(self, rep_teletext_stream, timestamp)
                log.debug(f"Opening DASH reader for: {rep_teletext_stream.ident!r} - {rep_teletext_stream.mimeType}")
                teletext.open()
                fds.append(teletext)
                metadata["s:t:{0}".format(_)] = ["language={0}".format(rep_teletext_stream.lang), "title=\"{0}\"".format(rep_teletext_stream.lang)]
            maps.extend(f"{_}:t" for _ in range(next_map, next_map + len(rep_teletext)))

        if rep_closed_captions and rep_closed_captions[0] and rep_video:
            for _, rep_cc in enumerate(rep_closed_captions):
                cc = DASHStreamReaderSUB(self, rep_cc, timestamp)
                log.debug(f"Opening DASH reader for: {rep_cc.ident!r} - {rep_cc.mimeType}")
                cc.open()
                fds.append(cc)
                metadata["s:c:{0}".format(_)] = ["language={0}".format(rep_cc.lang), "title=\"{0}\"".format(rep_cc.lang)]
            maps.extend(f"{_}:c" for _ in range(next_map, next_map + len(rep_closed_captions)))
...
def parse_args():
    ...
    parser.add_argument("-teletext", action="store_true", help="Enable support for teletext (if available)")
    parser.add_argument("-closed-captions", action="store_true", help="Enable support for closed captions (if available)")
...
def main():
    ...
    if args.subtitles:
        log.info(f"Mux Subtitles: Enabled")
        session.set_option("mux-subtitles", True)

    if args.teletext:
        log.info(f"Mux Teletext: Enabled")
        session.set_option("mux-teletext", True)

    if args.closed_captions:
        log.info(f"Mux Closed Captions: Enabled")
        session.set_option("mux-closed-captions", True)
    ...
    if clearkey:
        ...
        if args.teletext:
            plugin.options["use-teletext"] = True
        if args.closed_captions:
            plugin.options["use-closed-captions"] = True
        try:
